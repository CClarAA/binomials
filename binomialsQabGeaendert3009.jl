...
function saturateAxis(I::Singular.sideal,J::Singular.sideal)
	#input: two ideals in the same ring, J is generated by variables 
	#output: array with two entries: the first is the saturation of I with respect to J
	#the second is an integer k for which I:J^k=I:J^(k+1)=I:J^\infty
	check_parent(I,J)
	k=0
       	Saturation=Singular.satstd(I,J)
      	while isSubset(Saturation,I)==false
           	I=Singular.quotient(I,J)
         	k=k+1
		println(k)
       	end
       	return [Saturation,k]
...
function intersectionArray(A::Array{Singular.sideal,1})
	...
	for i=1:size(A,1)
		...
		#hopefully this step can be omited sometime
